...
server.listen({
	onUnhandledRequest: ({ method, url }) => {
		if (
			!url.href.startsWith('http://localhost:2222') &&
			!url.href.startsWith('http://localhost:5555/')
		) {
			throw new Error(
				`[MSW] Warning: captured a request without a matching request handler: Unhandled ${method} request to ${url}`,
			)
		}
	},
})
